...
router.get('/auth',userController.authCheck); // указываем get запрос для проверки access токена,авторизован ли пользователь,в данном случае это отдельный эндпоинт вместо authMiddleware,делаем все проверки как в authMiddleware,только уже в самой функции эндпоинта(просто в данном случае нужна только проверка как в authMiddleware)
